...
conn = oracledb.connect(user='DIGITALIZACION', password='DIGITALIZACION', dsn=dsn)
```
...
# Modo individual (para GUI o consultas singulas)
node scripts/batch_oracle.js --cedula=1714797782 --fecha=2026-04-01

# Modo batch (desde Oracle)
node scripts/batch_oracle.js --limit=50
```

Parametros:
- `--cedula=XXXX` + `--fecha=YYYY` - Modo individual, genera PDF para una cedula
- `--limit=N` - Modo batch, consulta N registros desde Oracle

El script:
1. Conecta a Oracle con thick mode
2. Para cada registro: consulta portal MSP + genera PDF
3. Aplica delay de 2s entre consultas
4. Reintenta hasta 3 veces en caso de error 503
